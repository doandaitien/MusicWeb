						...
						<tr>
							...
							<td>
								<div class="nut" style="height: 20px;">
									<a href="<?= base_url();?>Admin/xoaUser/<?=$value['AID']?>" class="btn btn-default delete">Xóa <i class="fas fa-times"></i></a>
								</div>
							</td>
						</tr>
						<?php endforeach?>
			...
			<div class="row text-center page">
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
					<ul class="pagination justify-content-center" style="margin: 30px 0">
						<li class="page-item">
							<a href="<?= base_url();?>Admin/page/<?= $trangHienTai - 1?>" class="page-link">Previous</a>
						</li>
						<?php for($i = 0; $i < $sotrang; $i++) {?>
							<?php if($i+1 == $trangHienTai) {?>
								<li class="page-item active"><a class="page-link" href="<?= base_url(); ?>Admin/page/<?= $i+1 ?>"><?= $i+1?></a></li>
							<?php } else {?>
								<li class="page-item"><a class="page-link" href="<?= base_url(); ?>Admin/page/<?= $i+1 ?>"><?= $i+1?></a></li>
							<?php } ?>
						<?php } ?>
						<li class="page-item">
							<a href="<?= base_url();?>Admin/page/<?= $trangHienTai + 1?>" class="page-link">Next</a>
						</li>										
					</ul>
				</div>
			</div>
